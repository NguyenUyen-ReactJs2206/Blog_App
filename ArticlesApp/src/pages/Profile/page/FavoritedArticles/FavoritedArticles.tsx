...
export default function FavoritedArticles() {
  ...
  useEffect(() => {
    const promise = dispatch(getListFavoriteArtileThunk(queryConfig))
    setPagination((prev) => ({
      ...prev,
      totalPage: Math.ceil(Number(favoritedArticles.articlesCount) / PAGINATION.LIMIT)
    }))
    return () => {
      promise.abort()
    }
  }, [queryConfig, dispatch, favoritedArticles.articlesCount])
  ...
  return (
    <div className=''>
      ...
      {favoritedArticles.articles.length === 0 && <div className='py-4'>Not Favorited Articles...</div>}
